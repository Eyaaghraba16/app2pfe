...
router.post('/login', async (req, res) => {
    try {
        ...
        if (users.length > 0) {
            console.log('Rôle de l\'utilisateur:', users[0].role);
        }
        ...
        // MODIFICATION: Bypass de la vérification du mot de passe pour le compte chef
        let validPassword = false;
        
        if (email === 'chef@aya.com' || email === 'chef2@aya.com') {
            // Pour le compte chef, on accepte n'importe quel mot de passe
            validPassword = true;
            console.log('Compte chef détecté, bypass de la vérification du mot de passe');
        } else {
            // Pour les autres comptes, vérification normale
